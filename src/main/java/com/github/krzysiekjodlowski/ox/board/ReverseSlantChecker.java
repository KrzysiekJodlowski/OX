...
class ReverseSlantChecker implements LineChecker {
  ...
  public boolean checkLine(Move playersMove) {
    ...
    int found = 1;
    boolean checkBefore = ((current - this.board.getBoardSideLength()) > 0) && (current % this.board.getBoardSideLength() != 0);
    boolean checkAfter = ((current - 1) % this.board.getBoardSideLength() != 0) && ((current + this.board.getBoardSideLength() - 1) <= this.board.getBoardCapacity());
    ...
    for (int i = 1; i <= x; i++) {
      ...
      try {
        if (checkBefore && checkIfThereIsAnother((current - i * this.board.getBoardSideLength()) + i - 1, (current - i * this.board.getBoardSideLength()) + i, playersMove)) {
          found += 1;
        } else {
          checkBefore = false;
        }
      ...
      try {
        if (checkAfter && checkIfThereIsAnother(current + i * this.board.getBoardSideLength() - i, (current + i * this.board.getBoardSideLength()) - i, playersMove)) {
          found += 1;
        } else {
          checkAfter = false;
        }
